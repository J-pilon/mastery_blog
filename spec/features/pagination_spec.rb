...
RSpec.feature 'Paginations', type: :feature do
  ...
  before(:each) do
    visit signin_path
    within('form[data-testid=signin-form]') do
  ...
  xscenario 'shows default number of articles' do
    ...
    visit articles_path
    expect(page).to have_selector('div[data-testid=article-container]'), count: 10
  end
  ...
  xcontext 'when selecting' do
    scenario '10 per page, 10 articles are displayed', js: true do
      ...
      expect(page).to have_selector('div[data-testid=article-container]', count: 11)
      page.select('10', from: 'per-page-selector')
      expect(page).to have_selector('div[data-testid=article-container]', count: 10)
    end
    ...
    scenario '25 per page, 25 articles are displayed', js: true do
      ...
      visit articles_path
      page.select('25', from: 'per-page-selector')
      expect(page).to have_selector('div[data-testid=article-container]', count: 25)
    end

    scenario '50 per page, 50 articles are displayed', js: true do
      FactoryBot.create_list(:article, 50)
      visit articles_path
      page.select('50', from: 'per-page-selector')
      expect(page).to have_selector('div[data-testid=article-container]', count: 50)
    end

    scenario '100 per page, 100 articles are displayed', js: true do
      FactoryBot.create_list(:article, 100)
      visit articles_path
      page.select('100', from: 'per-page-selector')
      expect(page).to have_selector('div[data-testid=article-container]', count: 100)
    end

    scenario 'all, all articles are displayed', js: true do
      FactoryBot.create_list(:article, 15)
      visit articles_path
      page.select('all', from: 'per-page-selector')
      expect(page).to have_selector('div[data-testid=article-container]', count: 15)
    end
  end
